...
# ========== DNS-over-HTTPS を無効化 ==========
Write-Host "DNS-over-HTTPS（DoH）設定中..."
try {
    ...
    Write-Host "DNS-over-HTTPS の無効化完了"
} catch {
    Write-Warning "DNS-over-HTTPS の設定に失敗しました: $_"
}

# ========== Windows Update の実行 ==========
Write-Host "Windows Update を実行中..."
try {
    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force
    Install-Module PSWindowsUpdate -Force
    Get-WindowsUpdate -AcceptAll -Install -AutoReboot
} catch {
    Write-Warning "Windows Update の実行に失敗しました: $_"
}

# ========== 完了メッセージと再起動オプション ==========
Write-Host ""
Write-Host "セットアップが完了しました。"
Write-Host "日本語UIとIMEを有効にするには再起動が必要です。"

# Custom Script Extension では対話不可なので以下をコメントアウトまたは削除
# $choice = Read-Host "今すぐ再起動しますか？ (Y/N)"
# if ($choice -match '^[Yy]') {
